...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    changeProductCount: (state, action) => {
      ...
      if (product) {
        product.count = count;
        state.cart.totalPrice = state.cart.products.length > 0
          ? state.cart.products.reduce((total, item) => total + item.price * item.count, 0)
          : 0;
      }
    },
    deleteProductFromCart: (state, action) => {
      ...
      state.cart.totalPrice = state.cart.products.reduce((total, item) => total + item.price * item.count, 0);
    },
  },
});
